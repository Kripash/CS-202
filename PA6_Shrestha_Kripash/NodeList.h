//This file was given to us by Professor PaPaChristos to make our project easier for us.
#ifndef NODELIST_H
#define NODELIST_H
  ...
  public:
	...
	Node(const DataType& data, Node* next = NULL){
    	...
    	DataType& getData(){    //gets non-const reference, can be used to modify 					value of underlying data
				...
				//an alternative implementation, for reference
		return m_data;
    	}

        //gets the m_next memory address and returns it to access the next node.
	Node * getNext(){
	 	return m_next;
	}
	//gets the m_next memory address and returns it to access the next node.
	...
	//sets the m_next memory address of the next node so it can be accessed in 		the future.
